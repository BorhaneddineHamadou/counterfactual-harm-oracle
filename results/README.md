...
| `rq1/<subject>_calibration.npz` | `analysis/rq1_calibration.py --rerun` | conformal coverage, width, MAE per repeat of the shipped log-harm readout (`_rerun` = second execution, identical) |
...
| `rq2/table2.json`, `rq2/predictive_validity.json` | `analysis/rq2_predictive_validity.py` | Table 2 and its paired tests (600 held-out runs per subject) |
...
| `rq3/injury_curves*.json` | `analysis/rq3_injury_curves.py` | re-scoring under fourteen published curves (ordering stability, r_s lead over the telemetry scalars); `--sweep` = the re-distillation sweep |
